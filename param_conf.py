...
model_small = LecanemabModel(model_type, 5, (24*14), (12*1080), median_patient=True, param_type='noiiv')
model_small()
...
for i in range(n):
    ...
    models = [model_bw, model_m, model_small]
    for model in models:
    ...
    solver_small = Solution(model_small, 0, (12*1080), 1) 
    # Save values
    ...
    solutions_small = solver_small.solve()

    if i == 0:
        time = solutions_small.t
        results_small['time'] = time

    biomarker_small = solutions_small.y[2]
    central_L_small = solutions_small.y[0]
    peripheral_L_small = solutions_small.y[1]

    dict_of_cols_small['{}{}'.format(model_type, i)] = biomarker_small
    print(i)
...
results_small = pd.concat([results_small, pd.DataFrame(dict_of_cols_small)], axis=1)
...
results.to_csv("output/new10biweekly_{}_param_{}.csv".format(n, model_type), index=False)
...
results_m.to_csv("output/new10monthly_{}_param_{}.csv".format(n, model_type), index=False)

total_df_small = \
    results_small[list(results_small.filter(regex=model_type))]
results_small["Average"] = total_df_small.median(axis=1)
results_small["fifth"] = total_df_small.quantile(q=0.05, axis=1)
results_small["ninety-fifth"] = total_df_small.quantile(q=0.95, axis=1)

results_small.to_csv("output/new5biweekly{}_param_{}.csv".format(n, model_type), index=False)
...
mean_SUVr_small = results_small['Average']
fifth_SUVr_small = results_small['fifth']
ninety_fifth_small = results_small['ninety-fifth']
...
# plt.plot(time, mean_SUVr_small, color = 'red', label='2.5 mg/kg monthly')
# plt.fill_between(time, fifth_SUVr_small, 
#                  ninety_fifth_small, 
...
plt.show()
plt.savefig('plots/ab_new_biweekly_monthly_{}_param.png'.format(n))